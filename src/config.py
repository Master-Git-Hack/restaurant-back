...
class __Base(object):
    ...
    HOST: str = environ.get("HOST", "http://localhost:3000")

    class JWT:

        encode_params: dict = dict(
            key=environ.get("SECRET_KEY", uuid4()),
            algorithm=environ.get("ALGORITHM", "HS512"),
        )
        decode_params: dict = dict(
            key=environ.get("SECRET_KEY", uuid4()),
            algorithms=str(environ.get("ALGORITHMS", "HS512,")).split(","),
        )
